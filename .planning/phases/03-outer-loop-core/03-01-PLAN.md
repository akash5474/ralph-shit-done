...
must_haves:
  truths:
    ...
    - "Loop can determine next plan after current plan completes"
  artifacts:
    - path: "bin/ralph.sh"
    ...
    - path: "bin/lib/parse.sh"
      ...
      min_lines: 50
      exports: ["parse_next_task", "find_plan_file", "get_plan_name", "get_next_plan_after"]
  key_links:
    - from: "bin/ralph.sh"
      to: "bin/lib/state.sh"
    ...
    - from: "bin/lib/parse.sh"
      ...
      via: "grep for plan ID in Description line"
      pattern: "grep -oE '[0-9]{2}-[0-9]{2}'"
    - from: "bin/lib/parse.sh"
      to: ".planning/ROADMAP.md"
      via: "parse uncompleted plan checkboxes"
      pattern: "grep.*\\[ \\].*PLAN\\.md"
---
...
- Extract plan ID using: `grep -oE '[0-9]{2}-[0-9]{2}'`
  - This pattern matches "03-01" from "Execute plan 03-01 (first plan in Phase 3)"
  - Returns the first match (the plan ID)
- Return "COMPLETE" if no next task found or Description line is empty/missing
- Return plan ID on success

**find_plan_file()**
- Args: plan_id (e.g., "03-01")
- Extract phase number from plan_id: `phase_num="${plan_id%%-*}"`
- Find the phase directory matching that number using glob:
  `phase_dir=$(ls -d .planning/phases/${phase_num}-* 2>/dev/null | head -1)`
- Return full path to PLAN.md file: `${phase_dir}/${plan_id}-PLAN.md`
- Handle both zero-padded (03) and unpadded (3) folder names

**get_plan_name()**
- Args: plan_id (e.g., "03-01")
- Find the plan file and extract objective
- Parse first line after <objective> tag using sed
- Return short description (first ~50 chars)

**get_next_plan_after()**
- Args: current_plan_id (e.g., "03-01")
- Parse ROADMAP.md to find uncompleted plan checkboxes: `- [ ] NN-MM-PLAN.md`
- Use: `grep -E '^\s*- \[ \] [0-9]{2}-[0-9]{2}-PLAN\.md' .planning/ROADMAP.md`
- Extract all uncompleted plan IDs in order
- Find first plan ID that comes AFTER current_plan_id (numerically)
- Comparison logic:
  ```bash
  # Extract phase and plan numbers for comparison
  current_phase="${current_plan_id%%-*}"
  current_plan="${current_plan_id##*-}"
  # A plan comes after if: (phase > current_phase) OR (phase == current_phase AND plan > current_plan)
  ```
- Return next plan ID, or "COMPLETE" if current is the last uncompleted plan

Follow existing lib/ patterns:
- Use STATE_FILE variable (default .planning/STATE.md)
- Use ROADMAP_FILE variable (default .planning/ROADMAP.md)
...
get_next_plan_after "03-01"  # Should return "03-02"
get_next_plan_after "03-03"  # Should return "04-01" or next uncompleted
```
  </verify>
  <done>parse.sh exports parse_next_task, find_plan_file, get_plan_name, and get_next_plan_after functions that correctly parse STATE.md and ROADMAP.md</done>
</task>
...
- Determine next task using get_next_plan_after and call update_next_action:
  ```bash
...
```
  ...
  <done>ralph.sh logs iterations to ralph.log and updates STATE.md history/progress after each iteration, including advancing to next plan via get_next_plan_after</done>
</task>
...
3. `source bin/lib/parse.sh && get_next_plan_after "03-01"` returns "03-02"
4. ralph.log file is created with iteration entries
5. STATE.md iteration history updates correctly
6. Script handles --start-from flag
</verification>

<success_criteria>
- ralph.sh is executable and runs the main loop
- parse.sh correctly extracts next task from STATE.md using `grep -oE '[0-9]{2}-[0-9]{2}'`
- parse.sh can determine next plan after current via get_next_plan_after
